...
export class RotinasComponent implements OnInit {
  ...
  atalhosFiltrados = computed(() => {
    ...
    const filtrados = !termo
      ? [...atalhos]
      : atalhos.filter(atalho => this.normalizarTexto(`${atalho.nome} ${atalho.url} ${atalho.observacoes}`).includes(termo));

    return this.ordenarAtalhosPorFixacao(filtrados);
  });
  ...
  salvarAtalho(): void {
    ...
    const atualizados = this.ordenarAtalhosPorFixacao([atalho, ...this.atalhosDaRotina()]);
  ...
  excluirAtalho(event: MouseEvent, atalho: AtalhoRotina): void {
    ...
    const atualizados = this.ordenarAtalhosPorFixacao(this.atalhosDaRotina().filter(item => item.id !== atalho.id));
    this.atalhosDaRotina.set(atualizados);
    this.salvarAtalhosDaRotina(rotina, atualizados);
  }
  ...
  alternarFixado(event: MouseEvent, atalho: AtalhoRotina): void {
    ...
    const atualizados = this.ordenarAtalhosPorFixacao(this.atalhosDaRotina().map(item => item.id === atalho.id ? { ...item, fixado: !item.fixado } : item));
    this.atalhosDaRotina.set(atualizados);
    this.salvarAtalhosDaRotina(rotina, atualizados);
  }
  ...
  private carregarAtalhosDaRotina(rotina: Rotina): AtalhoRotina[] {
    ...
    if (salvo) {
      try {
        ...
        if (Array.isArray(atalhos)) {
          return this.ordenarAtalhosPorFixacao(atalhos);
        }
      } catch {
    ...
    const iniciais = this.ordenarAtalhosPorFixacao(this.criarAtalhosIniciais(rotina));
    this.salvarAtalhosDaRotina(rotina, iniciais);
    return iniciais;
  }

  private salvarAtalhosDaRotina(rotina: Rotina, atalhos: AtalhoRotina[]): void {
    const atalhosOrdenados = this.ordenarAtalhosPorFixacao(atalhos);
    localStorage.setItem(this.chaveStorageAtalhos(rotina), JSON.stringify(atalhosOrdenados));
  }
